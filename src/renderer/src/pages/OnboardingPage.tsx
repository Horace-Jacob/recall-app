  ...
  const handleProcessBookmarks = async (): Promise<void> => {
    ...
    try {
      for (let i = 0; i < selectedBookmarks.length; i++) {
        ...
        setProcessingProgress(Math.round((completed / selectedBookmarks.length) * 100));
      ...
      <div className="w-full max-w-2xl">
        ...
        {currentStep === 'complete' && result && (
          <div className="text-center space-y-8 animate-in fade-in duration-500">
            ...
            <p className="text-sm text-secondary-foreground mt-8">Redirecting to homepage...</p>
          </div>
        )}
      </div>
    </div>
